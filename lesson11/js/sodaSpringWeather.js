const apiURL = "http://api.openweathermap.org/data/2.5/weather?id=5607916&units=imperial&appid=ffe8fd08fbf90ef22ca04d78481b8036"
fetch(apiURL)
...
const forecastApiURL = "http://api.openweathermap.org/data/2.5/forecast?id=5607916&units=imperial&appid=ffe8fd08fbf90ef22ca04d78481b8036"
fetch(forecastApiURL)
  .then((response) => response.json())
  .then((jsObject) => {
    const forecast = jsObject.list.filter(day => day.dt_txt.includes("18:00:00"));
...
fetch(requestURL)
  ...
  .then(function (jsonObject) {
    ...
    sodaSprings = document.querySelector('.sodasprings-events')
    for (let i = 0; i < towns[6].events.length; i++ ) {
        let p = document.createElement('p') 
        p.textContent = towns[6].events[i];
        sodaSprings.appendChild(p);
      }
  });